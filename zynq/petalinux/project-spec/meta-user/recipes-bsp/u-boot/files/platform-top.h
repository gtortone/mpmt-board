...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"bootargs=earlyprintk earlycon clk_ignore_unused cpuidle.off=1 uio_pdrv_genirq.of_id=generic-uio cma=320M\0" \
	...
	"bootmmc=setenv bootargs ${bootargs} root=/dev/mmcblk0p1 rw rootwait && load mmc 0 ${kernel_load_address} /boot/zImage && load mmc 0 ${devicetree_load_address} /boot/system.dtb && bootz ${kernel_load_address} - ${devicetree_load_address}\0" \
	"bootmedia=mmc\0" \
